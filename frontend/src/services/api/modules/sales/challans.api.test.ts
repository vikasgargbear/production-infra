...
const readback = {
  ...
  lines: [{
    ...
    ledger_entry_id: id('10'), ledger_base_quantity: '3.000000',
  }],
};
...
describe('canonical sales-dispatch readback', () => {
  it('preserves exact quantity and lineage evidence without values or a selling total', () => {
    ...
    expect(normalizeCanonicalSalesDispatchReadback(readback)).not.toHaveProperty('inventory_value');
    expect(normalizeCanonicalSalesDispatchReadback(readback).lines[0]).not.toHaveProperty('ledger_value');
  });
