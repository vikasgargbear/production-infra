...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, onTabChange }) => {
			...
			<div className="space-y-1">
				{menuItems.map((item) => {
					...
					return (
						<button
							key={item.id}
							className={`w-full flex items-center px-3 py-2 rounded-lg text-sm transition-colors ${isActive
						...
						>
							...
							{!collapsed && (
								<div className="ml-3 flex-1 flex items-center justify-between">
									...
									{item.count && (
										<span className={`text-xs px-1.5 py-0.5 rounded-full border ${isActive
	...
	return (
		<>
			...
			{/* Mobile sidebar */}
			<div className={`fixed top-0 left-0 h-screen bg-white border-r border-gray-200 z-50 shadow-xl transition-all duration-300 transform md:hidden ${mobileOpen ? 'translate-x-0' : '-translate-x-full'
				} w-64`}>
			...
			{/* Desktop sidebar */}
			<div className={`hidden md:block h-screen bg-white border-r border-gray-200 shadow-sm transition-all duration-300 fixed top-0 left-0 z-10 ${collapsed ? 'w-16' : 'w-64'
				}`}>
				<div className="flex flex-col h-full">
					<SidebarHeader />
					<div className="p-3 overflow-y-auto flex-1">
