...
interface Customer {
  ...
  state?: string;
  gst_number?: string;
  customer_type?: 'retail' | 'wholesale' | 'hospital' | 'clinic' | 'distributor' | 'other';
  credit_limit?: number;
...
interface CustomerFormElements extends HTMLFormControlsCollection {
  ...
  contact_person: HTMLInputElement;
  phone: HTMLInputElement;
  email: HTMLInputElement;
  address: HTMLInputElement;
  city: HTMLInputElement;
  state: HTMLInputElement;
  gst_number: HTMLInputElement;
  customer_type: HTMLSelectElement;
  credit_limit: HTMLInputElement;
  payment_terms: HTMLInputElement;
}

interface CustomerFormElement extends HTMLFormElement {
...
const Customers: React.FC = () => {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent<CustomerFormElement>): Promise<void> => {
    e.preventDefault();
    ...
    try {
      ...
      for (let [key, value] of formData.entries()) {
        ...
        if (['credit_limit'].includes(key) && value !== '') {
          (customerData as any)[key] = parseFloat(value as string);
        } else {
  ...
  const handleDelete = useCallback(async (customerId: number): Promise<void> => {
  ...
  const CustomerModal: React.FC = () => {
    ...
    useEffect(() => {
      if (editingCustomer && formRef.current) {
        ...
        const elements = form.elements as CustomerFormElements;
        
        // Set initial values for all fields when editing
        if (editingCustomer.customer_name) elements.customer_name.value = editingCustomer.customer_name;
        if (editingCustomer.contact_person) elements.contact_person.value = editingCustomer.contact_person;
        if (editingCustomer.phone) elements.phone.value = editingCustomer.phone;
        if (editingCustomer.email) elements.email.value = editingCustomer.email;
        if (editingCustomer.address) elements.address.value = editingCustomer.address;
        if (editingCustomer.city) elements.city.value = editingCustomer.city;
        if (editingCustomer.state) elements.state.value = editingCustomer.state;
        if (editingCustomer.gst_number) elements.gst_number.value = editingCustomer.gst_number;
        if (editingCustomer.customer_type) elements.customer_type.value = editingCustomer.customer_type;
        if (editingCustomer.credit_limit !== undefined) elements.credit_limit.value = editingCustomer.credit_limit.toString();
        if (editingCustomer.payment_terms !== undefined) elements.payment_terms.value = editingCustomer.payment_terms.toString();
      }
    }, [editingCustomer]);
