...
import {
  ...
  CheckCircle, ArrowLeft, History as HistoryIcon
} from 'lucide-react';
import {
...
const StockWriteOffFlow = ({ onClose }) => {
  ...
  const [expiringProducts, setExpiringProducts] = useState<WriteOffItem[]>([]);
  ...
  const loadExpiringStock = async () => {
    setLoading(true);
    try {
      const response = await stockApi.getExpiringBatches({
        days_ahead: 90,
      ...
      if (response.data) {
        ...
        const transformedItems = items.map(item => ({
          ...
          days_to_expiry: Math.ceil((new Date(item.expiry_date).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
        }));
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <div className="text-right">
                    ...
                    <DatePicker
                      value={new Date(writeOffData.write_off_date)}
                      onChange={(date: Date) => setWriteOffData(prev => ({
                        ...prev,
                        write_off_date: date ? date.toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
                      }))}
                      className="mt-1"
                    />
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(value) => setWriteOffData(prev => ({ ...prev, reason: value as string }))}
