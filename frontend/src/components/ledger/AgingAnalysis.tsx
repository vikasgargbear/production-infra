...
const AgingAnalysis: React.FC<AgingAnalysisProps> = ({ open = true, onClose }) => {
  ...
  const loadAgingAnalysis = async (): Promise<void> => {
    setLoading(true);
    try {
      ...
      const analysisData: AgingParty[] = response.data.aging_analysis || [];
  ...
  const handleExportAnalysis = async (): Promise<void> => {
    try {
  ...
  const getAgingStatusColor = (daysOverdue: number): string => {
  ...
  const AgingBarChart: React.FC<{ party: AgingParty }> = ({ party }) => {
