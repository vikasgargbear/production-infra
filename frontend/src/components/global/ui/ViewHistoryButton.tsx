...
import {
    History, Eye, Edit, Download, Printer, X, Search,
...
type HistoryType = 'invoice' | 'challan' | 'payment' | 'purchase' | 'returns' | 'stock' | 'order' | 'sales-order';
...
export interface ViewHistoryButtonProps {
    ...
    onEditItem?: (item: HistoryItem) => void;
    className?: string;
    buttonText?: string;
}

export interface ViewHistoryButtonRef {
    click: () => void;
}

// ==================== COMPONENT ====================

const ViewHistoryButton = forwardRef<ViewHistoryButtonRef, ViewHistoryButtonProps>(({
    historyType = 'invoice',
    onViewItem,
    onEditItem,
    className = '',
    ...
    const loadHistory = async (): Promise<void> => {
        ...
        try {
            let response: any;
            let formattedItems: HistoryItem[] = [];
            ...
            switch (historyType) {
                ...
                case 'challan':
                    try {
                        ...
                        formattedItems = challans.map((challan: any) => ({
                ...
                case 'purchase':
                    try {
                        response = await (purchasesApi as any).getAll({ limit: 20 });
                ...
                case 'returns':
                    try {
                        const [salesReturns, purchaseReturns] = await Promise.all([
                            (returnsApi as any).getSaleReturns(),
                            (returnsApi as any).getPurchaseReturns()
                        ]);
                        const salesData = salesReturns.data?.returns || [];
                        const purchaseData = purchaseReturns.data?.returns || [];
                        const allReturns: HistoryItem[] = [
                            ...salesData.map((ret: any) => ({
                                id: ret.id,
                        ...
                        formattedItems = allReturns.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                    } catch {
                        formattedItems = [];
                    }
                    break;
                ...
                case 'stock':
                    try {
                        response = await (stockApi as any).getMovements({ limit: 20 });
                        const movementData = response.data?.movements || response.data || [];
                        formattedItems = movementData.map((movement: any) => ({
                            id: movement.id,
                ...
                case 'sales-order':
                    ...
                    formattedItems = orderData.map((order: any) => ({
    ...
    const printPDF = (): void => {
        ...
        const pdfGenerator = new GlobalPDFGenerator('digital');
            ...
            >
                ...
                <div className="relative">
                    {loadSuccess ? (
                        ...
                        <History className={`w-4 h-4 transition-all duration-500 ${isHovered
                ...
                {buttonText && (
                    <span className={`relative ${loadSuccess
                        ? 'text-white'
                        : isHovered
                                                ...
                                                <div className="flex items-center space-x-4 flex-1">
                                                    ...
                                                    {item.status && (
                                                        <div className="min-w-[100px] text-center">
                                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${(item.status === 'paid' || item.status === 'completed')
                                                                ...
                                                                <span className={`w-2 h-2 rounded-full mr-1.5 ${(item.status === 'paid' || item.status === 'completed')
                                                                    ? 'bg-green-400'
                                                                    : (item.status === 'pending')
                                                                        ? 'bg-yellow-400'
                                                                        : (item.status === 'cancelled' || item.status === 'canceled')
