 ...
 */
const OfflineStockIndicator: React.FC = () => {
    const [pendingBatches, setPendingBatches] = useState<PendingBatch[]>([]);
    const { isOnline } = useNetworkStatus() as { isOnline: boolean };
    ...
    const loadPendingBatches = async (): Promise<void> => {
        try {
            const batches = await (offlineDB as any).getBatchesWithReservations();
            ...
            >
                ...
                >
                    <div className="flex items-center gap-2">
                        <div className={`p-2 rounded-full ${isOnline ? 'bg-amber-100' : 'bg-gray-100'}`}>
                            <Clock className={`w-4 h-4 ${isOnline ? 'text-amber-600' : 'text-gray-600'}`} />
                        </div>
                        <div>
