  ...
  const OrderModal = () => {
    ...
    useEffect(() => {
      if (editingOrder && formRef.current) {
        ...
        if (editingOrder.customer_id) form.customer_id.value = editingOrder.customer_id;
        if (editingOrder.order_date) form.order_date.value = editingOrder.order_date;
        if (editingOrder.delivery_date) form.delivery_date.value = editingOrder.delivery_date;
        if (editingOrder.status) form.status.value = editingOrder.status;
        if (editingOrder.payment_status) form.payment_status.value = editingOrder.payment_status;
        if (editingOrder.payment_mode) form.payment_mode.value = editingOrder.payment_mode;
        if (editingOrder.total_amount) form.total_amount.value = editingOrder.total_amount;
        if (editingOrder.discount) form.discount.value = editingOrder.discount;
        if (editingOrder.final_amount) form.final_amount.value = editingOrder.final_amount;
        if (editingOrder.notes) form.notes.value = editingOrder.notes;
      }
    }, [editingOrder]);
