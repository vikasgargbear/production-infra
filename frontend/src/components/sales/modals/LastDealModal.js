...
const LastDealModal = ({ isOpen, onClose, productId, productName, customerId }) => {
  ...
  const loadLastDeals = async () => {
    setLoading(true);
    setError(null);
    
    try {
      // Fetch last deals for this product (optionally filtered by customer)
      const response = await invoiceAPI.getLastDeals(productId, customerId);
