...
import { InvoiceSummary as Invoice } from './invoice/types/invoiceTypes';
import {
...
interface InvoiceListProps {
  onClose?: () => void;
  filterStatus?: string; // Initial filter status (e.g., 'pending')
}
...
const InvoiceList: React.FC<InvoiceListProps> = ({ onClose, filterStatus: initialFilter }) => {
      ...
      if (response.success) {
        ...
        const transformedInvoices = (response.data as any).invoices.map((invoice: any) => {
          ...
          return {
            ...
            order_number: invoice.order_number,
            order_date: invoice.order_date,
            items_count: invoice.items?.length || invoice.total_items || 0
          };
        });
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Payment',
      render: (value: string, invoice: Invoice) => {
        const paymentText = getStatusText(invoice.payment_status || invoice.paymentStatus);
        return (
          <StatusBadge
            status={paymentText}
            variant="light"
