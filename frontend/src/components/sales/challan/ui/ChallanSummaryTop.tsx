...
interface ChallanSummaryTopProps {
  challan: Challan;
  onChallanUpdate: (updates: Partial<Challan>) => void;
}

const ChallanSummaryTop: React.FC<ChallanSummaryTopProps> = ({ challan, onChallanUpdate }) => {
  const handleFieldChange = (field: keyof Challan, value: string) => {
    onChallanUpdate({ [field]: value });
  };

  const handleTransportFieldChange = (field: keyof TransportDetails, value: string | number) => {
    onChallanUpdate({
      transport_details: {
