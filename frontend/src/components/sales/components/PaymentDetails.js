...
const PaymentDetails = ({ 
  ...
  const balanceAmount = (parseFloat(totalAmount) || 0) - (parseFloat(paymentData.amount_paid) || 0);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        {/* Amount Paid */}
        {(paymentData.payment_mode !== 'credit' || paymentData.amount_paid > 0) && (
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-gray-600 mb-2">
              <DollarSign className="w-4 h-4" />
              Amount Paid
            ...
            {readOnly ? (
              <div className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-50 font-medium text-green-700">
                ₹{(parseFloat(paymentData.amount_paid) || 0).toFixed(2)}
              </div>
            ) : (
              <CurrencyInput
