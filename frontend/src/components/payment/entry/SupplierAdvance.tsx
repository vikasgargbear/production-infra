      ...
      {workspace === 'prepare' && <>
        ...
        {context && <section className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <label className="text-sm font-medium">Supplier<select aria-label="Supplier" value={supplierId} onChange={event => { invalidatePrepare(); setSupplierId(event.target.value); setPurchaseOrderId(''); setLineId(''); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select supplier</option>{context.suppliers.map(row => <option key={row.supplier_account_id} value={row.supplier_account_id}>{row.supplier_code} — {row.supplier_name}</option>)}</select></label>
            <label className="text-sm font-medium">Approved purchase order<select aria-label="Approved purchase order" value={purchaseOrderId} onChange={event => { invalidatePrepare(); const next = event.target.value; setPurchaseOrderId(next); const matches = supplier?.lines.filter(row => row.purchase_order_id === next) || []; setLineId(matches.length === 1 ? matches[0].purchase_order_line_id : ''); setAmount(''); }} disabled={!supplier} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select purchase order</option>{purchaseOrders.map(order => <option key={order.id} value={order.id}>{order.number} · {order.orderDate}</option>)}</select></label>
            <label className="text-sm font-medium">Approved PO product line<select aria-label="Approved PO product line" value={lineId} onChange={event => { invalidatePrepare(); setLineId(event.target.value); setAmount(''); }} disabled={!purchaseOrderId} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select PO line</option>{purchaseOrderLines.map(line => <option key={line.purchase_order_line_id} value={line.purchase_order_line_id}>line {line.line_number} · {line.product_code} · available ₹{line.remaining_advance_amount}</option>)}</select></label>
            <label className="text-sm font-medium">Organization payment date<input aria-label="Organization payment date" type="date" max={maximumDate || undefined} value={paymentDate} onChange={event => { const next = event.target.value; invalidatePrepare(); setPaymentDate(next); if (next) void loadContext(next); else setContext(null); }} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
            <label className="text-sm font-medium">Bank and settlement ledger<select aria-label="Bank and settlement ledger" value={bankId} onChange={event => { invalidatePrepare(); setBankId(event.target.value); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select INR bank</option>{context.bank_accounts.map(row => <option key={row.bank_account_id} value={row.bank_account_id}>{row.bank_name} — {row.account_holder_name} ({row.ifsc})</option>)}</select></label>
            <label className="text-sm font-medium">Method<select aria-label="Method" value={method} onChange={event => { invalidatePrepare(); setMethod(event.target.value as SupplierAdvanceMethod | ''); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select payment method</option><option value="upi">UPI</option><option value="bank_transfer">Bank transfer</option></select></label>
            <label className="text-sm font-medium">Bank / UPI reference<input aria-label="Bank / UPI reference" value={reference} onChange={event => { invalidatePrepare(); setReference(event.target.value); }} maxLength={256} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
            <label className="text-sm font-medium">Gross advance amount<input aria-label="Gross advance amount" inputMode="decimal" value={amount} onChange={event => { invalidatePrepare(); setAmount(event.target.value); }} placeholder="0.00" className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
          </div>
