...
const paymentId = 'd3000000-0000-7000-8000-000000000011';
const preview: any = {
  command_request_id: 'd3000000-0000-7000-8000-000000000010',
  preview_hash: `sha256:${'a'.repeat(64)}`,
};
const detail = (): any => ({ payment_id: paymentId, status: 'posted' });

test('retains the executed UUID before readback and retries with GET only', async () => {
  ...
  await expect(reconcileCanonicalSupplierAdvance(
...
test('fails closed on invalid execution identity', async () => {
  const retain = jest.fn();
  await expect(reconcileCanonicalSupplierAdvance(
    preview, 'stable', null, jest.fn().mockResolvedValue('legacy-1'), jest.fn(), retain,
  )).rejects.toThrow(/no valid supplier-advance identity/i);
