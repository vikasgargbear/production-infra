    ...
    <div className="flex h-full flex-col bg-slate-50">
      ...
      <main className="flex-1 overflow-auto p-5"><div className="mx-auto max-w-5xl space-y-4">
        ...
        {step === 'review' && preparedPayload && <section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-lg font-semibold">Confirm supplier payment</h2><p className="mt-1 text-sm text-slate-600">Posts one bank disbursement, debits accounts payable, credits the selected bank ledger, and allocates the listed invoices.</p>
          ...
          <dl className="mt-5 grid gap-3 rounded-lg bg-slate-50 p-4 md:grid-cols-2"><div><dt className="text-xs uppercase text-slate-500">Gross / cash</dt><dd className="font-semibold">₹{preparedPayload.gross_amount}</dd></div><div><dt className="text-xs uppercase text-slate-500">Reference</dt><dd className="font-semibold">{preparedPayload.external_reference}</dd></div><div><dt className="text-xs uppercase text-slate-500">Date</dt><dd>{preparedPayload.payment_date}</dd></div><div><dt className="text-xs uppercase text-slate-500">Allocations</dt><dd>{preparedPayload.allocations.length} posted invoice(s)</dd></div></dl>
