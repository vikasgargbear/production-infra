...
const CreditDebitFlow: React.FC<CreditDebitFlowProps> = ({ onClose, open = true, noteType = 'credit' }) => {
  ...
  const selectDocument = async (nextId: string) => {
    ...
    try {
      ...
      setContext(next);
      setRuleId(next.rule_choices[0]?.id || '');
    ...
    <main className="flex-1 overflow-auto p-5"><div className="mx-auto max-w-6xl space-y-4">
      ...
      {workspace === 'prepare' && <>
        ...
        {context && <section className="rounded-xl border border-slate-200 bg-white p-5">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2"><label className="text-sm font-medium">Reviewed GST rule<select value={ruleId} onChange={event => { setRuleId(event.target.value); invalidatePreparedDraft(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3">{context.rule_choices.map(rule => <option key={rule.id} value={rule.id}>{rule.reason_code} — {rule.gst_tax_treatment.replace('_', ' ')}</option>)}</select></label><label className="text-sm font-medium">Business reason<input value={reason} onChange={event => { setReason(event.target.value); invalidatePreparedDraft(); }} maxLength={1024} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label></div>
          {chosenRule?.gst_tax_treatment === 'statutory' && side === 'sales' && <div className="mt-4 grid gap-4 md:grid-cols-2"><label className="text-sm font-medium">ITC-reversal evidence attachment UUID<input value={recipientEvidenceId} onChange={event => { setRecipientEvidenceId(event.target.value); invalidatePreparedDraft(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label><label className="text-sm font-medium">ITC reversal confirmed at<input type="datetime-local" value={recipientConfirmedAt.slice(0, 16)} onChange={event => { setRecipientConfirmedAt(event.target.value ? new Date(event.target.value).toISOString() : ''); invalidatePreparedDraft(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label></div>}
