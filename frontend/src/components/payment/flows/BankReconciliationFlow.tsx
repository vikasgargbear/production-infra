...
const BankReconciliationFlow: React.FC<Props> = ({ onClose, open = true }) => {
  ...
  const [matchMethod, setMatchMethod] = useState<'manual' | 'reference_exact'>('manual');
  ...
  const chooseCandidate = (key: string) => {
    ...
    setMatchMethod(context?.candidates.find(row => candidateKey(row) === key)?.match_methods[0] || 'manual');
  };
  const prepare = async () => {
    if (!selected) return;
