...
const PrepareWorkspace = (props: any) => <>
  <section className="rounded-xl border border-slate-200 bg-white p-5">
    ...
    <div className="mt-4 grid gap-4 md:grid-cols-3">
      <label className="text-sm font-medium">Branch<select value={props.branchId} onChange={event => props.setBranchId(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select branch</option>{props.branches.map((branch: BranchChoice) => <option key={branch.branch_id} value={branch.branch_id}>{branch.branch_code} — {branch.branch_name}</option>)}</select></label>
  ...
  {props.context && <section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="text-lg font-semibold">Claim details</h2><p className="text-sm text-slate-600">Gross receipt expense only. GST input credit, withholding, FX, mileage, per diem, advances, reversals, and partial approval fail closed.</p><div className="mt-4 grid gap-4 md:grid-cols-3">
    ...
    <label className="text-sm font-medium">Period end<input type="date" min={props.periodStart} max={props.context.business_date} value={props.periodEnd} onChange={event => props.setPeriodEnd(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
    <label className="text-sm font-medium">Reimbursement liability<select value={props.reimbursementAccountId} onChange={event => props.setReimbursementAccountId(event.target.value)} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select account</option>{props.context.reimbursement_accounts.map((account: any) => <option key={account.account_id} value={account.account_id}>{account.account_code} — {account.account_name}</option>)}</select></label>
  </div><label className="mt-4 block text-sm font-medium">Business purpose<textarea value={props.purpose} onChange={event => props.setPurpose(event.target.value)} rows={2} maxLength={1024} className="mt-1 w-full rounded-lg border px-3 py-2" /></label></section>}
