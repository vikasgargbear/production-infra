...
const EnterprisePaymentEntry = ({ open, onClose }) => {
  ...
  const [allocationMethod, setAllocationMethod] = useState('fifo'); // fifo or manual
  ...
  const handlePartySelect = (customer) => {
    ...
    setErrors(prev => ({ ...prev, party: null }));
  ...
  // Removed old single payment method handler - using multi-payment methods now
      ...
      <div className="bg-white border-t border-gray-200 p-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <button
              onClick={handleSave}
              disabled={loading || !formData.party || !formData.paymentType || formData.paymentMethods.every(m => !m.method || !m.amount)}
