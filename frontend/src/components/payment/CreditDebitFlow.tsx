...
const CreditDebitFlow: React.FC<CreditDebitFlowProps> = ({ 
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      alert(`Error saving ${isCredit ? 'credit' : 'debit'} note: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setSaving(false);
    }
  };
  ...
  const canProceedToNextStep = () => {
    switch (currentStep) {
      case 1:
        return selectedCustomer && noteData.reason && noteData.settlement_type && noteData.selected_invoice && noteItems.length > 0;
      case 2:
        return true; // Review step - all validations done in step 1
      default:
        return false;
    }
  };
          ...
          <div className="max-w-6xl mx-auto px-6 py-6">
            ...
            {currentStep === 2 && (
              <div className="space-y-6">
                {/* Review Summary */}
                <Card>
                  <h3 className="text-lg font-semibold text-gray-900 mb-6">Review {isCredit ? 'Credit' : 'Debit'} Note</h3>
