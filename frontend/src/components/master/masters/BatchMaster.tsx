...
const BatchMaster: React.FC<BatchMasterProps> = ({ open, onClose }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const [filterStatus, setFilterStatus] = useState('all');
    const [showAddModal, setShowAddModal] = useState(false);
    const [editingBatch, setEditingBatch] = useState<Batch | null>(null);
    const [selectedBatches, setSelectedBatches] = useState<(number | string)[]>([]);
    ...
    const getBatchStatus = (expiryDate: string, availableQty: number) => {
    ...
    const loadProducts = async () => {
        try {
            ...
            if ((response as any)?.data && Array.isArray((response as any).data)) {
                setProducts((response as any).data.map((product: any) => ({
                    id: product.id,
