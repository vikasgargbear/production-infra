...
const StockTransfer = ({ open = true, onClose }: { open?: boolean; onClose: () => void }) => {
  ...
  const createContent = <div className="space-y-5">
    ...
    <section className="rounded-lg border border-gray-200 bg-white p-5" aria-labelledby="transfer-products-heading">
      ...
      {items.length === 0 ? <div className="mt-5 rounded-lg border border-dashed border-gray-300 p-8 text-center text-sm text-gray-500"><Package className="mx-auto mb-2 h-8 w-8" />No transfer lines yet.</div> : <>
        <div className="mt-5 space-y-3 md:hidden" data-testid="stock-transfer-mobile-lines">{items.map(item => <article key={item.productId} className="rounded-lg border border-gray-200 bg-white p-3"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h4 className="truncate font-semibold">{item.productName}</h4><p className="text-sm text-gray-500">{item.productCode}</p></div><button type="button" aria-label={`Remove ${item.productName}`} className="inline-flex min-h-12 min-w-12 items-center justify-center rounded-lg text-red-600 hover:bg-red-50" onClick={() => { setItems(current => current.filter(row => row.productId !== item.productId)); invalidatePreparedReview(); }}><X className="h-4 w-4" /></button></div><p className="mt-3 text-sm"><span className="text-gray-500">Requested</span> <strong>{item.requestedQuantity} {item.selectedUomCode}</strong></p><ul className="mt-2 space-y-2 text-sm">{item.allocations.map(allocation => <li key={allocation.batch.batch_id} className="rounded bg-gray-50 p-2"><strong>{allocation.batch.batch_number}</strong> · {allocation.enteredQuantity} {allocation.batch.selected_uom_code}<span className="block text-xs text-gray-500">Expires {allocation.batch.expires_on}</span></li>)}</ul></article>)}</div>
  ...
  const reviewContent = <div className="space-y-5" data-testid="canonical-immutable-preview">
    ...
    {confirmOpen && prepared && <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4" role="presentation" onKeyDown={(event) => { if (event.key === 'Escape' && !committing) setConfirmOpen(false); }}><div className="absolute inset-0 bg-black/50" onClick={() => !committing && setConfirmOpen(false)} /><div ref={confirmDialogRef} role="dialog" aria-modal="true" aria-labelledby="transfer-confirm-title" className="relative max-h-[90dvh] w-full max-w-lg overflow-y-auto rounded-t-2xl bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-2xl sm:rounded-xl sm:p-6"><AlertCircle className="h-6 w-6 text-amber-600" /><h3 id="transfer-confirm-title" className="mt-2 text-lg font-semibold">Confirm inter-branch stock transfer</h3><p className="mt-3 text-sm">Move {items.length} reviewed product(s) from <strong>{sourceBranch?.branch_name}</strong> to <strong>{destinationBranch?.branch_name}</strong>. The exact preview will be approved, then executed once.</p><p className="mt-3 break-all text-xs text-gray-500">Command: {prepared.command_request_id}</p><div className="sticky bottom-0 mt-6 flex gap-2 bg-white pt-3"><button type="button" disabled={committing} className="min-h-12 flex-1 rounded-lg border" onClick={() => setConfirmOpen(false)}>Cancel</button><button type="button" disabled={committing || executeAttempted} className="min-h-12 flex-[2] rounded-lg bg-blue-600 px-4 font-medium text-white disabled:bg-gray-300" onClick={commitOnce}>{committing ? 'Posting…' : executeAttempted ? 'Execution attempted' : 'Approve and post once'}</button></div></div></div>}
  </div>;
