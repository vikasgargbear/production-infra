    ...
    <main className="flex-1 overflow-auto p-5" data-testid="canonical-immutable-preview"><div className="mx-auto max-w-6xl space-y-4">
      ...
      {workspace === 'prepare' && <section className="rounded-xl border border-slate-200 bg-white p-5">
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-2">
          <label className="text-sm font-medium">Eligible full stock balance<select aria-label="Eligible full stock balance" value={candidateId} onChange={event => { setCandidateId(event.target.value); setReasonCode(''); invalidate(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select stock</option>{context?.candidates.map(row => <option key={`${row.batch_id}:${row.uom_conversion_id}`} value={`${row.batch_id}:${row.uom_conversion_id}`}>{row.branch_code} · {row.location_code} · {row.product_name} · {row.batch_number} · {row.available_selected_quantity} {row.selected_uom_code} · {formatExactCurrency(row.inventory_value, 'Inventory value')}</option>)}</select></label>
          <label className="text-sm font-medium">Verified destruction certificate<select aria-label="Verified destruction certificate" value={certificateId} onChange={event => { setCertificateId(event.target.value); invalidate(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select certificate</option>{context?.certificates.map(row => <option key={row.certificate_attachment_id} value={row.certificate_attachment_id}>{row.original_filename} · {row.document_date}</option>)}</select></label>
          <label className="text-sm font-medium">Verified Section 17(5)(h) reversal evidence<select aria-label="Verified Section 17(5)(h) reversal evidence" value={itcReversalEvidenceId} onChange={event => { setItcReversalEvidenceId(event.target.value); invalidate(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select reversal evidence</option>{context?.itc_reversal_evidence.map(row => <option key={row.certificate_attachment_id} value={row.certificate_attachment_id}>{row.original_filename} · {row.document_date}</option>)}</select></label>
          <label className="text-sm font-medium">Reason code<select aria-label="Reason code" value={reasonCode} onChange={event => { setReasonCode(event.target.value as ReasonCode); invalidate(); }} className="mt-1 min-h-11 w-full rounded-lg border px-3"><option value="">Select reviewed reason</option>{candidate && context && candidate.expires_on <= context.business_date && <option value="expired">Expired</option>}{candidate?.batch_status !== 'expired' && <><option value="damaged">Damaged</option><option value="quality_rejected">Quality rejected</option></>}</select></label>
          <label className="text-sm font-medium">Detailed reason<input aria-label="Detailed reason" value={reason} onChange={event => { setReason(event.target.value); invalidate(); }} maxLength={1024} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
          <label className="text-sm font-medium">Authority reference<input aria-label="Authority reference" value={authorityReference} onChange={event => { setAuthorityReference(event.target.value); invalidate(); }} maxLength={1024} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
          <label className="text-sm font-medium">Witness name<input aria-label="Witness name" value={witnessName} onChange={event => { setWitnessName(event.target.value); invalidate(); }} maxLength={1024} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
          <label className="text-sm font-medium">Witness credential<input aria-label="Witness credential" value={witnessCredential} onChange={event => { setWitnessCredential(event.target.value); invalidate(); }} maxLength={1024} className="mt-1 min-h-11 w-full rounded-lg border px-3" /></label>
          <label className="text-sm font-medium">Physical destruction completed at (UTC)<input aria-label="Physical destruction completed at (UTC)" type="text" value={physicalConfirmedAt} onChange={event => { setPhysicalConfirmedAt(event.target.value); invalidate(); }} placeholder="YYYY-MM-DDTHH:mm:ss.sssZ" aria-describedby="destruction-time-help" aria-invalid={Boolean(physicalConfirmedAt) && !isCanonicalUtcEventTimestamp(physicalConfirmedAt)} className="mt-1 min-h-11 w-full rounded-lg border px-3 font-mono" /><span id="destruction-time-help" className="mt-1 block text-xs font-normal text-slate-500">Enter the exact timestamp recorded by the retained certificate or witness evidence. The browser does not supply or convert this time.</span></label>
        </div>
