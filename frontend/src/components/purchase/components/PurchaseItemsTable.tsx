...
const PurchaseItemsTable: React.FC<PurchaseItemsTableProps> = () => {
  ...
  const handleAddProduct = (productWithBatch: ProductWithBatch): void => {
    ...
    const itemData = {
      ...
      mrp: parseFloat(String(batch.mrp || product.mrp || 0)).toFixed(2),
      tax_percent: product.gst_percent || product.tax_rate || PURCHASE_CONFIG.DEFAULTS.TAX_RATE,
      purchase_price: parseFloat(String(batch.ptr || product.cost_price || product.ptr || (Number(product.mrp) || 0) * 0.7)).toFixed(2),
    ...
    if (!batch.batch_number) {
      setTimeout(() => {
        ...
        if (lastItem) {
          const batchInput = document.querySelector(`#batch-${lastItem.id}`) as HTMLInputElement;
  ...
  const handleItemFieldChange = (itemId: string, field: string, value: string | number): void => {
    updateItem(itemId, field, value);
    
    // Validate item field
    const item = purchase.items && purchase.items.find((i: PurchaseItem) => i.id === itemId);
    if (item) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-fixed">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {purchase.items && purchase.items.map((item: PurchaseItem, index: number) => (
      ...
      {purchase.items && purchase.items.length > 0 && (
        <div className="px-4 py-3 bg-gray-50 border-t border-gray-200">
          <div className="flex items-center justify-end gap-8">
            ...
            <span className="text-sm text-gray-600">
              Qty: {purchase.items.reduce((sum: number, item: PurchaseItem) => sum + (parseInt(String(item.quantity)) || 0) + (parseInt(String(item.free_quantity)) || 0), 0)}
            </span>
            <span className="text-sm font-medium text-gray-900">
              Total: {formatCurrency(purchase.items.reduce((sum: number, item: PurchaseItem) => sum + calculateItemAmount(item), 0))}
            </span>
          </div>
        </div>
      )}
      ...
      <ProductCreationModal
        ...
        onProductCreated={(product: ProductWithBatch) => {
