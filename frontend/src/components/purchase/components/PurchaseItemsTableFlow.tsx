...
interface PurchaseItemsTableFlowProps {
  items: PurchaseItem[];
  onUpdateItem: (index: number, field: string, value: string | number) => void;
  onRemoveItem: (index: number) => void;
}

const PurchaseItemsTableFlow: React.FC<PurchaseItemsTableFlowProps> = ({ 
  items, 
  onUpdateItem, 
  onRemoveItem 
}) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>, index: number, field: string): void => {
    if (e.key === 'Enter') {
      e.preventDefault();
      // Find next input
      ...
      const inputs = currentRow.querySelectorAll('input, select') as NodeListOf<HTMLInputElement | HTMLSelectElement>;
      ...
      if (currentIndex < inputs.length - 1) {
        ...
        if (nextRow) {
          const firstInput = nextRow.querySelector('input, select') as HTMLInputElement | HTMLSelectElement;
        ...
        <table className="w-full">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {items.map((item, index) => {
              ...
              const gstPercent = parseFloat(String(item.tax_percent)) || 12;
