        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          <div>
            ...
            <span className="ml-2 font-medium">
              {purchase.items.reduce((sum: number, item: PurchaseItem) => sum + (parseFloat(String(item.quantity)) || 0), 0).toFixed(2)}
            </span>
          </div>
        </div>
      </div>
      ...
      {purchase.tax_amount > 0 && (
        <div className="mt-4 p-4 bg-blue-50 rounded-lg">
          ...
          <div className="space-y-1 text-sm">
            {(() => {
              const taxBreakdown = purchase.items.reduce((acc: { [key: string]: number }, item: PurchaseItem) => {
                const rate = item.tax_percent || 0;
                const quantity = parseFloat(String(item.quantity)) || 0;
                const price = parseFloat(String(item.purchase_price)) || 0;
                const tax = (quantity * price * Number(rate)) / 100;
