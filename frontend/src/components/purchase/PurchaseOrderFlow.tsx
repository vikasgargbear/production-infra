...
import { SupplierSearch, PurchaseProductSearch, ItemsTable, NotesSection, ProductCreationModal, GSTCalculator, ViewHistoryButton, ModuleHeader, StandardDatePicker, DocumentFooter, BaseModal } from '../global';
...
const PurchaseOrderFlow = ({ onClose, prefilledData = null }) => {
  ...
  const [purchaseOrder, setPurchaseOrder] = useState<any>({
    ...
    // Reference details
    reference_no: (prefilledData as any)?.reference_no || '',
    quotation_no: '',
    requisition_no: '',
...
5. Goods once accepted will not be returned except for quality issues`,
  ...
  const handleSavePO = async () => {
    ...
    try {
      ...
      const errorMessage = (error as any).response?.data?.detail || (error as any).message || 'Failed to create purchase order';
...
${localStorage.getItem('company_name') || 'AASO Pharmaceuticals'}
  ...
  const sendEmailWithGmail = async (emailData: any): Promise<void> => {
    const orderDetails = purchaseOrder.items.map((item: any) =>
      `${item.product_name} - Qty: ${item.quantity} @ ₹${item.purchase_price}`
    ).join(', ');
      ...
      <div className="h-full bg-blue-50">
        <div className="h-full flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto" ref={poFormRef}>
            <div className="max-w-6xl mx-auto px-6 py-6 space-y-6">
              {message && (
                <div className={`rounded-lg p-4 flex items-center gap-3 ${messageType === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'
                  }`}>
                  {messageType === 'error' ? (
              ...
              <div className="bg-white rounded-lg shadow-sm border border-blue-200 p-6">
                ...
                {!selectedSupplier ? (
                  <div>
                    {/* @ts-ignore - SupplierSearch props mismatch */}
                    <SupplierSearch
              ...
              <div className="bg-white rounded-lg shadow-sm border border-blue-200 p-6">
                ...
                {purchaseOrder.items.length > 0 ? (
                  /* @ts-ignore - ItemsTable props mismatch */
                  <ItemsTable
                    ref={itemsTableRef}
        ...
        {showSupplierModal && (
          <SupplierCreationModal
            ...
            onSupplierCreated={(supplier: any) => {
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-6xl mx-auto px-6 py-6">
            {message && (
              <div className={`rounded-lg p-4 flex items-center gap-3 mb-6 ${messageType === 'error' ? 'bg-red-50 text-red-700' : 'bg-green-50 text-green-700'
                }`}>
                {messageType === 'error' ? (
                  <AlertCircle className="w-5 h-5" />
                ) : (
