...
describe('active canonical purchase desktop facts', () => {
    const activeSources = () => [
        ...
        read('purchase-order/PurchaseOrderFlow.tsx'),
        ...
        read('grn/CanonicalGoodsReceiptForm.tsx'),
        read('grn/canonicalReceiptCommand.ts'),
        read('../../services/api/modules/purchase/canonicalPurchaseOrders.api.ts'),
    ].join('\n');
    ...
    it('keeps calculation requests behind explicit canonical evidence', () => {
        ...
        const purchaseOrderCommand = read('purchase-order/utils/canonicalPurchaseOrderCommand.ts');
        expect(purchaseOrderCommand).toContain('requiredUuid');
        expect(purchaseOrderCommand).toContain('canonicalDecimal');
        expect(read('../../services/api/modules/purchase/canonicalPurchaseOrders.api.ts'))
            .toContain('requireCanonicalPurchaseOrderReadback');
    });
    ...
    it('retires browser tax catalogs and legacy purchase editors', () => {
        ...
        expect(existsSync(join(__dirname, 'PDFVerificationFlow.tsx'))).toBe(false);
        expect(existsSync(join(__dirname, 'modals/ProductVerificationModal.tsx'))).toBe(false);
        expect(existsSync(join(__dirname, 'modals/SupplierVerificationModal.tsx'))).toBe(false);
        expect(existsSync(join(__dirname, 'ui/PurchaseItemEditModal.tsx'))).toBe(false);
        expect(existsSync(join(__dirname, 'utils/productItemTransform.ts'))).toBe(false);
        expect(existsSync(join(__dirname, '../../services/calculations/purchaseOrderCalculationService.ts'))).toBe(false);
        expect(existsSync(join(__dirname, '../../services/api/modules/purchase/calculations.api.ts'))).toBe(false);
    ...
    it('uses one canonical GST fact and never turns missing API money into zero', () => {
        ...
        expect(read('purchase-order/utils/canonicalPurchaseOrderCommand.ts'))
            .not.toContain("gst_type: order.gst_type || 'CGST/SGST'");
