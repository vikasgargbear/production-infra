...
interface ProductLineEntryProps {
  item: ProductItem;
  index: number;
 ...
 */
const ProductLineEntry: React.FC<ProductLineEntryProps> = ({ item, index, onUpdate, onRemove }) => {
  const [searchTerm, setSearchTerm] = useState<string>(item.product_name || '');
  ...
  const selectProduct = (product: SearchProduct): void => {
  ...
  useEffect(() => {
    if (item.mrp && !item.selling_price) {
      ...
      const mrpNum = typeof item.mrp === 'number' ? item.mrp : parseFloat(String(item.mrp)) || 0;
      onUpdate({ selling_price: (mrpNum * 0.9).toFixed(2) });
    }
  }, [item.mrp]);
      ...
      <td className="px-3 py-2 relative" ref={searchRef}>
        <div className="relative">
          <input
            ...
            className={`w-full px-2 py-1 border rounded ${(item.validation?.errors?.length ?? 0) > 0 ? 'border-red-500' : 'border-gray-300'
