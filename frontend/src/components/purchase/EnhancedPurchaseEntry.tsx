...
interface EnhancedPurchaseEntryProps {
  onClose: () => void;
  prefilledData?: Partial<PurchaseData> | null;
}

const EnhancedPurchaseEntry: React.FC<EnhancedPurchaseEntryProps> = ({ onClose, prefilledData = null }) => {
  ...
  const calculateTotals = () => {
    ...
    (purchase.items || []).forEach(item => {
      ...
      if (item.product_id || item.product_name) {
        ...
        const purchasePrice = parseFloat(String(item.purchase_price || item.rate || item.cost)) || 0;
      ...
      items: (prev.items || []).map((item, i) => {
        if (i === index) {
          ...
          if (field === 'rate') {
            ...
            updatedItem.cost_price = value; // Sync cost_price
          } else if (field === 'tax') {
            ...
            updatedItem.sale_price = value; // Sync sale_price
          } else if (field === 'sale_price') {
            updatedItem.selling_price = value; // Sync selling_price
          }
          return updatedItem;
        }
        return item;
      ...
      const purchaseData = {
        ...
        items: purchase.items.map((item, index) => {
          ...
          return {
            ...
            quantity: parseFloat(String(item.quantity)) || 1, // This will be transformed to ordered_quantity
            free_quantity: parseFloat(String(item.free_quantity)) || 0,
            purchase_price: parseFloat(String(item.purchase_price)) || 0, // This will be transformed to cost_price
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {purchase.items.map((item, index) => {
                  ...
                  const cost = parseFloat(String(item.purchase_price)) || parseFloat(String(item.cost_price)) || 0;
                  ...
                  return (
                    <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="text-right py-2 px-2">
                        <NumberInput
                          value={item.purchase_price || item.cost_price || null}
                          onChange={(val) => handleUpdateItem(index, 'purchase_price', val)}
