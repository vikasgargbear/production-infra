...
type FilterStatus = 'all' | 'matched' | 'mismatched' | 'missing';

const GSTReconciliation: React.FC<GSTReconciliationProps> = ({ open, onClose }) => {
  const [activeTab, setActiveTab] = useState<ActiveTab>('purchase');
  const [searchTerm, setSearchTerm] = useState<string>('');
  const [filterStatus, setFilterStatus] = useState<FilterStatus>('all');
  
  // Mock reconciliation data
  const reconciliationData: Record<ActiveTab, ReconciliationData> = {
    purchase: {
      matched: 145,
      mismatched: 23,
      missing: 12,
