...
BEGIN
  ...
  FOR party_fact IN
   ...
   WHERE fact.org_id=organization_id AND fact.dataset_id=reviewed_dataset_id
     ...
     AND (
       fact.selection_state<>'quarantined'
       OR EXISTS (
         SELECT 1 FROM automation.historical_migration_facts opening
          WHERE opening.org_id=fact.org_id AND opening.dataset_id=fact.dataset_id
            AND opening.source_kind='opening_item'
            AND opening.selection_state<>'quarantined'
            AND (
              opening.party_key=fact.party_key
              OR opening.party_key=fact.payload->>'source_party_id'
              OR opening.payload->>'source_party_id'=fact.party_key
              OR opening.payload->>'source_party_id'=fact.payload->>'source_party_id'
            )
       )
     )
  ...
  LOOP
    ...
    party_source_key:=COALESCE(
      NULLIF(pg_catalog.btrim(party_fact.payload->>'source_party_id'),''),
      NULLIF(pg_catalog.btrim(party_fact.party_key),'')
    );
    IF role_name NOT IN ('customer','supplier')
       OR party_source_key IS NULL
    ...
    INSERT INTO automation.historical_party_bindings(
      ...
      organization_id,reviewed_dataset_id,party_source_key,role_name,party_fact.id,
    ...
    LOOP
      ...
      SELECT binding.party_id INTO STRICT party_identifier
       ...
       WHERE binding.org_id=organization_id AND binding.dataset_id=reviewed_dataset_id
         AND binding.source_party_id IN (
           opening_fact.party_key,
           opening_fact.payload->>'source_party_id'
         );
