...
AS $function$
    ...
    canonical_identity AS MATERIALIZED (
        SELECT reference.*,
          ...
          JOIN catalog.products AS product
           ...
           AND product.id = reference.product_id
           AND product.row_version = reference.product_row_version
          JOIN parties.parties AS manufacturer
            ON manufacturer.org_id = product.org_id
           AND manufacturer.id = product.manufacturer_party_id
         WHERE manufacturer.row_version >= 1
           AND pg_catalog.btrim(manufacturer.legal_name) <> ''
    )
    SELECT reference.product_id, reference.product_row_version,
           reference.product_code, reference.product_name,
           pg_catalog.btrim(reference.manufacturer_name)
      FROM canonical_identity AS reference
     WHERE reference.product_reference_count = 1
       ...
       AND (SELECT count(*) FROM canonical_identity) = (
           SELECT count(*) FROM invoice_products
       )
       AND EXISTS (
           SELECT 1
