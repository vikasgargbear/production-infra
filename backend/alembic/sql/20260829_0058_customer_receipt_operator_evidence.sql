-- Typed immutable evidence lifecycle for customer receipts.
...
ALTER TABLE core.attachments
  ...
  CHECK (
    ...
    OR (
      ...
      AND evidence_kind IN ('expense_receipt','customer_receipt_evidence')
...
CREATE FUNCTION erp_core_commands.initiate_customer_receipt_attachment(
...
BEGIN
  ...
  IF erp_security.has_permission('finance.payment.manage',branch_identifier) IS DISTINCT FROM true THEN
    RAISE EXCEPTION USING ERRCODE='42501', MESSAGE='customer receipt evidence permission denied';
  END IF;
  IF attachment_identifier IS NULL OR branch_identifier IS NULL
     ...
     OR storage_object_path IS DISTINCT FROM organization_id::text||'/'||branch_identifier::text
        ||'/customer_receipt_evidence/'||pg_catalog.encode(sha256,'hex')||'.pdf'
    ...
    RAISE EXCEPTION USING ERRCODE='22023', MESSAGE='customer receipt evidence metadata is invalid';
  END IF;
  ...
  IF existing_count=1 THEN
    ...
    IF existing.evidence_kind<>'customer_receipt_evidence'
  ...
  INSERT INTO core.attachments(
    ...
    'customer_receipt_evidence',document_date,retention_until,false,'pending_upload',actor_id
  );
...
CREATE FUNCTION erp_core_commands.transition_customer_receipt_attachment(
  organization_id uuid,
  branch_identifier uuid,
  attachment_identifier uuid,
  target_status text
...
BEGIN
  ...
  IF target_status IS NULL OR target_status NOT IN ('verified','rejected') THEN
    RAISE EXCEPTION USING ERRCODE='22023', MESSAGE='customer receipt evidence transition is invalid';
  END IF;
  SELECT * INTO current_attachment FROM core.attachments attachment
   ...
   FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION USING ERRCODE='P0002', MESSAGE='customer receipt attachment not found';
  END IF;
...
ALTER FUNCTION erp_core_commands.initiate_customer_receipt_attachment(
  uuid,uuid,uuid,text,text,text,bigint,bytea,date,date
) OWNER TO erp_migration_owner;
ALTER FUNCTION erp_core_commands.transition_customer_receipt_attachment(
  uuid,uuid,uuid,text
) OWNER TO erp_migration_owner;
REVOKE ALL ON FUNCTION erp_core_commands.initiate_customer_receipt_attachment(
  uuid,uuid,uuid,text,text,text,bigint,bytea,date,date
) FROM PUBLIC,erp_app,erp_runtime;
REVOKE ALL ON FUNCTION erp_core_commands.transition_customer_receipt_attachment(
  uuid,uuid,uuid,text
) FROM PUBLIC,erp_app,erp_runtime;
GRANT EXECUTE ON FUNCTION erp_core_commands.initiate_customer_receipt_attachment(
  uuid,uuid,uuid,text,text,text,bigint,bytea,date,date
) TO erp_runtime;
GRANT EXECUTE ON FUNCTION erp_core_commands.transition_customer_receipt_attachment(
  uuid,uuid,uuid,text
) TO erp_runtime;
