...
def ipv4_resolver(host: str, port: int, *, family: int, type: int):
...
def test_verifier_proves_caller_ipv4_role_and_rls_without_fallback() -> None:
    ...
    connection = FakeConnection(
        ("erp_runtime", False, False, False, True, "postgres")
    )
    ...
    assert "pooler.supabase.com" not in connections[0]
...
@pytest.mark.parametrize(
    "posture",
    [
        ("wrong_role", False, False, False, True, "postgres"),
        ("erp_runtime", True, False, False, True, "postgres"),
        ("erp_runtime", False, True, False, True, "postgres"),
        ("erp_runtime", False, False, True, True, "postgres"),
        ("erp_runtime", False, False, False, False, "postgres"),
        ("erp_runtime", False, False, False, True, "wrong_database"),
    ],
)
def test_verifier_rejects_role_or_rls_posture(posture: tuple[object, ...]) -> None:
    with pytest.raises(
        ...
        DATABASE.verify_direct_database(
            ...
            application_name="canonical_direct_verify",
            resolver=ipv4_resolver,
            connect=lambda _dsn: FakeConnection(posture),
        )
...
def test_verifier_rejects_missing_ipv4_before_connecting() -> None:
    ...
    with pytest.raises(
        DATABASE.CanonicalStagingDatabaseError,
        match="no caller-visible IPv4 resolution",
    ):
        DATABASE.verify_direct_database(
            contract=contract(),
            role="erp_runtime",
