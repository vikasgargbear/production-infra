...
def test_free_staging_uses_only_manifest_owned_direct_ipv4_database_transport():
    ...
    assert "allowlisted diagnostic unavailable" in workflow
    assert 're.sub(r"postgres(?:ql)?://\\S+", "<redacted-database-url>", line)' in workflow
    assert "diagnostic_lines[-4:]" in workflow
...
def test_whole_database_reset_is_retired_and_org_purge_is_two_run_scoped():
    ...
    assert "EXPECTED_CANONICAL_RELATION_COUNT = 124" in reset_authority
    assert "EXPECTED_EPHEMERAL_RELATION_COUNT = 9" in reset_authority
    assert "EXPECTED_ORGANIZATION_RELATION_COUNT = 108" in reset_authority
