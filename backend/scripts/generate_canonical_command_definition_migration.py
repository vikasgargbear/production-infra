...
def generate_sql() -> str:
    statements = [
        _function_definition(relative_path, schema, function)
        for relative_path, schema, function in FUNCTION_TARGETS
    ]
    statements.extend(
        _constraint_definition(*target) for target in CONSTRAINT_TARGETS
    )
    statements = [
    ...
    return (
        ...
        + ";\n".join(statements)
        + ";\nDROP TRIGGER IF EXISTS tax_portal_documents_immutable_trg "
        "ON tax.portal_documents;\nRESET ROLE;\n"
    )
