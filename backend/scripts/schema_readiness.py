...
This tool is deliberately read-only. It does not connect to a database. It evaluates the effective mounted
    ...
    *,
    ...
    if reset != {
        ...
        issues.append(Issue(
            ...
            message="Schema authority requires reset-only with every legacy compatibility mode disabled.",
            path=classification_path,
        ))
...
def audit_authority_contract(repo_root: Path) -> tuple[Issue, ...]:
    """Validate canonical-only authority and classify every competing source."""
    root = repo_root.resolve()
    authority = load_authority(root)
    issues: list[Issue] = []
    issues.extend(check_migration_infrastructure(authority, root))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    mode.add_argument(
        ...
        action="store_true",
        help=(
            "Validate reset-only migration authority and competing-source classification."
        ),
    )
    ...
    try:
        if args.validate_authority:
            ...
            print(
                ...
                "(canonical Alembic only; reset-only strategy enforced)"
            )
            return 0
