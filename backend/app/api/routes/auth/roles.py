...
IMPORTANT: Static path routes (/setup-defaults, /assign, /validate, /user/...)
must be defined BEFORE parameterized routes (/{role_id}) to avoid FastAPI
matching "user" or "assign" as a role_id.
"""
...
def get_user_permissions(
    ...
    Fetching another user's permissions requires master:view.
    """
    try:
        org_id = current_user.get('org_id')
        requesting_user_id = current_user.get('user_id')

        # Allow users to fetch their own permissions without master:view
        if user_id != requesting_user_id:
            # Check if requesting user has master:view
            is_admin = current_user.get('is_admin', False)
            if not is_admin:
                role_manager = RoleManager(db)
                if not role_manager.validate_permission(requesting_user_id, 'master', 'view'):
                    raise HTTPException(
