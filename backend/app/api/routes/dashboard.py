...
def get_data_quality(
    ...
    """Get data quality metrics for master data"""
    try:
        # Calculate completeness for products
        product_query = """
            SELECT 
                ...
                COUNT(CASE WHEN gst_rate IS NOT NULL THEN 1 END) as with_gst,
                COUNT(CASE WHEN manufacturer IS NOT NULL AND manufacturer != '' THEN 1 END) as with_manufacturer
            FROM inventory.products WHERE is_active = true
        """
        
        product_result = db.execute(text(product_query))
        product_stats = dict(product_result.first()._mapping)
        
        # Calculate completeness for customers
        customer_query = """
            SELECT 
                COUNT(*) as total_customers,
                COUNT(CASE WHEN customer_name IS NOT NULL AND customer_name != '' THEN 1 END) as with_name,
        ...
        customer_result = db.execute(text(customer_query))
        customer_stats = dict(customer_result.first()._mapping)
        
        # Calculate overall completeness percentage
        total_fields = 0
        filled_fields = 0
        
        if product_stats['total_products'] > 0:
            total_fields += product_stats['total_products'] * 4  # 4 key fields
            filled_fields += product_stats['with_name'] + product_stats['with_hsn'] + product_stats['with_gst'] + product_stats['with_manufacturer']
        
        if customer_stats['total_customers'] > 0:
            total_fields += customer_stats['total_customers'] * 4  # 4 key fields
            filled_fields += customer_stats['with_name'] + customer_stats['with_phone'] + customer_stats['with_email'] + customer_stats['with_gst']
        
        completeness = (filled_fields / total_fields * 100) if total_fields > 0 else 0
        
        # Calculate accuracy (simplified - checking for valid formats)
        accuracy_query = """
            SELECT 
                (SELECT COUNT(*) FROM parties.customers WHERE email LIKE '%@%.%') as valid_emails,
                (SELECT COUNT(*) FROM parties.customers WHERE email IS NOT NULL AND email != '') as total_emails,
                (SELECT COUNT(*) FROM parties.customers WHERE gst_number ~ '^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9A-Z]{1}Z[0-9A-Z]{1}$') as valid_gst,
                (SELECT COUNT(*) FROM parties.customers WHERE gst_number IS NOT NULL AND gst_number != '') as total_gst
        """
        
        accuracy_result = db.execute(text(accuracy_query))
        accuracy_stats = dict(accuracy_result.first()._mapping)
        
        accuracy = 85  # Base accuracy
        if accuracy_stats['total_emails'] > 0:
            accuracy = accuracy * (accuracy_stats['valid_emails'] / accuracy_stats['total_emails'])
        if accuracy_stats['total_gst'] > 0:
            accuracy = accuracy * (accuracy_stats['valid_gst'] / accuracy_stats['total_gst'])
        
        # Consistency check (simplified)
        consistency = 92  # Base consistency - would need more complex checks in production
...
def get_system_health(
    ...
    try:
        ...
        sync_result = db.execute(text(sync_query))
        last_activity = sync_result.scalar()
        ...
        last_backup = datetime.now() - timedelta(hours=2)  # Assume backup was 2 hours ago
...
def get_recent_activity(
    limit: int = Query(20, description="Number of activities to fetch"),
    db: Session = Depends(get_db),
    org_id: str = Depends(get_org_id_from_header)
):
        ...
        query = """
            WITH recent_activities AS (
                ...
                SELECT 
                    ...
                    p.product_id::text as activity_id,
                    'Product added: ' || p.product_name as description,
                    p.manufacturer as entity_name,
                    NULL as value,
                ...
                SELECT 
                    ...
                    c.customer_id::text as activity_id,
