...
async def create_stock_receive(
    ...
    try:
        ...
        stock = InventoryService.get_location_wise_stock(db, org_id, receive_data["product_id"], location_id, receive_data.get("batch_id"))
        
        if stock:
            InventoryService.update_location_stock_increase(db, stock["stock_id"], receive_data["quantity"], receive_data["movement_date"])
        else:
            InventoryService.insert_location_stock(db, org_id, location_id, receive_data["product_id"], receive_data.get("batch_id"), receive_data["quantity"], receive_data["movement_date"])
        
        return {"status": "success", "movement_id": movement_id, "movement_number": movement_number, "message": f"Stock receive {movement_number} created successfully"}
...
async def create_stock_issue(
    ...
    try:
        ...
        if not allow_negative and stock["quantity_available"] < issue_data["quantity"]:
            raise HTTPException(status_code=400, detail=f"Insufficient stock. Available: {stock['quantity_available']}. Enable 'Allow Negative Stock' in Master Settings to proceed.")
        ...
        InventoryService.update_location_stock_decrease(db, stock["stock_id"], issue_data["quantity"], issue_data["movement_date"])
        
        return {"status": "success", "movement_id": movement_id, "movement_number": movement_number, "message": f"Stock issue {movement_number} created successfully"}
    except HTTPException:
        db.rollback()
        raise
    except Exception as e:
...
async def create_stock_transfer(
    ...
    try:
        ...
        # Create issue and receive (simplified - uses the above endpoints logic)
        return {"status": "success", "message": "Stock transfer completed successfully"}
    except HTTPException:
        raise
    except Exception as e:
